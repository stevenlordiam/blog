...
function generate() {
  ...
  const content = []
  fs.readdirSync(path.join(__dirname, 'pages', 'novels')).forEach(function(file) {
    content.push({
      content: fs.readFileSync(path.join(__dirname, 'pages', 'novels', file)).toString(),
      url: `/novels/${file.replace('.mdx', '')}`
    })
  })
